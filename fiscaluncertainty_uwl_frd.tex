...
\author[James Murray, UW-L Faculty Research Day]
{
James Murray\\
...
{
  \ft{Purpose}
  \begin{block}{Two Part Goal}
    \bi
    \item Quantify fiscal policy uncertainty (FPU)
    \item Determine what effect FPU has on health of the economy
    \ei
  \end{block}

  \begin{block}{What is fiscal policy?}
    \bi
    \item Fiscal policy: Decisions on government expenditure, tax rates, transfers, and accumulated debt.
    \item U.S. data that aggregates fiscal variables over federal, state and local governments.
    \item Usually think of \textit{fiscal policy} as movements in fiscal variables to target economic outcomes.
    \item Aggregate of \textit{discretionary} and \textit{automatic} fiscal policy.
    \ei
  \end{block}
}
...
\frame
{
  \ft{Quantifying Fiscal Policy Uncertainty}
  ...
  \begin{block}{Present Paper}
      \bi
      \item Market participants behave like statisticians - they estimate fiscal policy rules.
...
\frame
{
  ...
  \begin{block}{Fiscal Policy Variables}
    ...
    \begin{column}{0.5\textwidth}
        \textit{Regression model (i.e. learning process) for each.}\\
\ \\
  ...
  Include these measures of fiscal uncertainty in a vector autoregression (VAR) model including:
    \be
    \item Consumption
    \item Investment
    \item Real GDP
...
\frame
{
  \ft{Learning About Fiscal Policy}
  \begin{block}{Expectations Example: Government Spending}
  \bi
  \item When unemployment is high, increase government spending to boost the economy.
  \item When spending is low, increase government spending to boost the economy.
  \item When accumulated debt is high, decrease government spending to keep debt level sustainable.
  \item Levels of government spending today depend on yesterday's level (\textit{persistent}).
  \ei
  \end{block}

  \begin{block}{Learning Process / Expectations Mechanism}
    \bi
    \item Run a regression that determines how fiscal policy variables react to unemployment, spending, debt, previous policy.
    \item Every period, run a regression for each fiscal policy variable, using data from previous periods.
    \ei
  \end{block}
}
...
\frame
{
 ...
 \begin{block}{Fiscal Policy Uncertainty}
    \bi
    \item Regression will give you a prediction for the fiscal policy variable.
    \item Also be confronted with reality: actual fiscal policy is realized
    \item Difference $\equiv$ Forecast error - fiscal policy that was not predicted / not in line with past behavior
    \item Larger accumulated forecast errors imply greater uncertainty
    \ei
  \end{block}
